...
{
#ifdef OPENBOX_S4	
	return fast_reg_rd(FAST_RULE_REG_WADDR+addr);
#else
	u64 value = 0;
...
{
#ifdef OPENBOX_S4	
	fast_reg_wr(FAST_RULE_REG_WADDR+addr,value);//原来使用的规则写法，正常		
#else
...
delete_rule:
	...
	ap = table.rules[idx].priority;
	ap = (ap<<32)|table.rules[idx].valid;	
