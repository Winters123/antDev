...
void send_eth_packet_out(u32 inport,u32 outport,struct eth_header *eth,int len,int hit_idx)
{
	...
	{		
		memset(eth,0x00,1);/*LLDP报文首字节为1时，有时libnet发送完成后，在模拟环境中接收不到，故改为0，接收时还原*/
	}
...
void *aging_rule_sw(void *argv)
{
	...
	while(1)
	{
		...
		{
			if(aging_rule_time[i].tv_sec != 0)
			{
				temp_stats = fast_reg_rd(i*0x60000);/*要替换成每流对应的计数器*/

				if(flow_stats[i]!= 0 && flow_stats[i] != temp_stats)/*如果计数有变化，则更新新的老化时间，时间长度存在tv_usec里*/
				{
					//printf("Aging (%d) ADD TIME!\n",i);
					aging_rule_time[i].tv_sec += aging_rule_time[i].tv_usec;/*tv_usec存储着老化时间长度*/
				}
				flow_stats[i] = temp_stats;
				if(tv.tv_sec > aging_rule_time[i].tv_sec)/*到了老化时间～*/
				{
					//printf("Aging %d-%d > %d ----------------\n",i,tv.tv_sec,aging_rule_time[i].tv_sec);
					...
					fast_del_rule(i);
					fast_reg_wr(FAST_ACTION_REG_ADDR + i*8,ACTION_SET_MID<<28|FAST_DMID_PROTO_STACK);
					//print_sw_rule_by_idx(i);
				}		
				else
				{
					//printf("----------------------[%d]sub time:%d\n",i,aging_rule_time[i].tv_sec - tv.tv_sec);
				}
			}
		}
		sleep(1);
...
{	
	...
	for(;i<OFP_MAX_RULE_CNT;i++)
	{
		if(flow_stats_addr[i] != NULL)
		{
			...
			((struct ofp_flow_stats *)flow_stats_addr[i])->packet_count = htonll(fast_reg_rd(0x60000|(i*0x10)|0x08));
			((struct ofp_flow_stats *)flow_stats_addr[i])->byte_count = htonll(fast_reg_rd(0x60000|(i*0x10)));
